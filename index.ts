...
nextapp.prepare().then(() => {
    ...
    app.use(expressSession({ secret: '3649db42b5c25f90e79c96b9bb2454daae172731', resave: true, saveUninitialized: true }))
    app.use(passport.initialize())
    app.use(passport.session())

    const dbname = 'issuetracker'
    const mongo_url:string = process.env.MONGO_CONNECTION_STRING?process.env.MONGO_CONNECTION_STRING:'mongodb://localhost:27017'
    // @ts-ignore
    ...
    mongo.then( (client:mongodb.MongoClient)=>{
        ...
        passport.use(new LocalStrategy(
            ...
            compared.then(isPasswordCorrect=>{
        ...
        app.get("/api/issues", (req, res) => {
            if(!req.user) {res.status(401).type('json').send({status:'Unauthorized'})}
            else
            db.collection('issues').find().sort( { date: -1 } )
        ...
        app.post("/api/new", (req, res) => {
            if(!req.user) res.status(401).send('Unauthorized')
                ...
                if(!title || !description || !db){
                    ...
                    const sanitizedTitle = title.match(re).join('').trim()
                    const sanitizedDescription = description.match(re).join('').trim()
                    db.collection('issues').insertOne({
                        title:sanitizedTitle.length<=150?sanitizedTitle:(sanitizedTitle.substring(0,150)+"..."),
                        description:sanitizedDescription.length<=1500?sanitizedDescription:(sanitizedDescription.substring(0,1500)+"..."),
                        date: new Date(),
                        status:'open',
                        user:req.user
        ...
        app.get('*', (req, res) => {
            ...
            if (route) {
                if(pathname==='/'){
                    ...
                    res['issues'] = db.collection('issues').find().sort( { date: -1 } )
                    //                .limit(10)
